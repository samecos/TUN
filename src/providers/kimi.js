        ...
        onRawChunk: (rawData) => {
          ...
          for (const msg of messages) {
            try {
              ...
              // 工具调用状态（如搜索中）
              if (data.op === 'set' && data.block?.tool?.status === 'STATUS_RUNNING') {
                if (!searchNotified) {
                  searchNotified = true;
                  if (!started) {
                    started = true;
              ...
              // 文本块初始化
              if (data.op === 'set' && data.mask === 'block.text' && data.block?.text?.content) {
                if (!started) {
                  started = true;
                  callbacks.onStart();
                }
              ...
              // 文本追加（核心！）
              if (data.op === 'append' && data.mask === 'block.text.content' && data.block?.text?.content) {
                if (!started) {
                  started = true;
                  callbacks.onStart();
                }
              ...
              // 消息完成
              if (data.op === 'set' && data.message?.status === 'MESSAGE_STATUS_COMPLETED' &&
