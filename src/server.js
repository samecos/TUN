  ...
  try {
    ...
    for (let i = 0; i < pages.length; i++) {
      ...
      const domInfo = await p.evaluate(() => {
        ...
        info.contentEditables = Array.from(document.querySelectorAll('[contenteditable="true"]'))
          .map(el => ({ tag: el.tagName, class: el.className.substring(0, 100), id: el.id }));
        ...
        // 所有 textarea
        info.textareas = Array.from(document.querySelectorAll('textarea'))
          .map(el => ({ class: el.className.substring(0, 100), placeholder: el.placeholder }));

        // 遮罩层
        info.masks = Array.from(document.querySelectorAll('.mask, [class*="overlay"], [class*="modal"]'))
          .map(el => ({ tag: el.tagName, class: el.className.substring(0, 100), visible: el.offsetWidth > 0 }));

        // 消息容器候选
        const msgSelectors = [
          '[class*="message"]', '[class*="chat"]', '[class*="segment"]',
          '[class*="response"]', '[class*="answer"]', '[class*="reply"]',
          '[data-role]', '.markdown-body',
        ];
        ...
        for (const sel of msgSelectors) {
          ...
          if (count > 0) {
            ...
            if (lastEl && lastEl.textContent.trim()) {
              info.messageContainers[sel + '_lastText'] = lastEl.textContent.trim().substring(0, 100);
            }
          }
        }
...
async function handleClientMessage(username, ws, msg) {
  switch (msg.type) {
    case 'new_chat': {
      console.log(`[Chat] ${username} 开启新对话: ${msg.provider}/${msg.feature} (${msg.conversationId})`);
      sessionManager.createConversation(username, msg.conversationId, msg.provider, msg.feature);
    ...
    case 'chat_message': {
      ...
      console.log(`[Chat] ${username} -> ${conv.provider}/${conv.feature}: ${msg.content.substring(0, 50)}...`);

      // 如果浏览器管理器可用，发送到真实的 AI 服务
      if (browserManager && browserManager.isReady()) {
        try {
          await browserManager.sendMessage(conv.provider, conv.feature, msg.content, msg.conversationId, {
            onStart: () => {
              ws.send(JSON.stringify({
          ...
          ws.send(JSON.stringify({ type: 'error', message: '发送消息失败: ' + err.message }));
        }
      } else {
        // 开发模式：模拟回复
